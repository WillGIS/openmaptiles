...
RETURNS TABLE(geometry geometry, render_height int, render_min_height int, query_height numeric) AS $$
    SELECT  geometry,  render_height::int, render_min_height::int, query_height::numeric FROM (
        -- etldoc: fangwujianzhu ->  layer_tj_building:z13
        SELECT   geometry, fw_g as render_height, fw_g as query_height, gc as render_min_height FROM tj_fangwu_fangwujianzhu WHERE geometry && bbox AND zoom_level = 13
        UNION ALL
        -- etldoc: fangwujianzhu -> layer_tj_building:z14
        SELECT   geometry, fw_g as render_height, fw_g as query_height, gc as render_min_height FROM tj_fangwu_fangwujianzhu WHERE geometry && bbox AND zoom_level >= 14
    ) AS zoom_levels;

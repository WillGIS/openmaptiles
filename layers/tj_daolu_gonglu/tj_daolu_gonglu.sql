...
RETURNS TABLE(geometry geometry, name text , road_type text, areacode int) AS $$
    SELECT geometry, name, road_type, areacode::int FROM (
        -- etldoc: fangwujianzhu -> layer_tjbuilding:z13
        SELECT geometry, name, type as road_type, areacode FROM tj_daolu_gonglu2 WHERE geometry && bbox AND zoom_level = 12 AND zoom_level > 8
        UNION ALL 
        -- etldoc: fangwujianzhu -> layer_tjbuilding:z13
        SELECT geometry, name, type as road_type, areacode FROM tj_daolu_gonglu2 WHERE geometry && bbox AND zoom_level = 13
        UNION ALL

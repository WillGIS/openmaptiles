...
RETURNS TABLE(geometry geometry, typecode text, name text, objectid int, level_f int, level_t int) AS $$
    SELECT geometry, typecode, name, objectid::int, level_f::int, level_t::int FROM (
